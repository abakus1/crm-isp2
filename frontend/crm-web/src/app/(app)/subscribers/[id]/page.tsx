    ...
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-xs text-muted-foreground">{formatKind(s.kind)} · {formatStatus(s.status)} · ID: {s.id}</div>
        </div>
      ...
      <div className="rounded-xl border bg-card p-4 space-y-3">
        <div className="text-xs text-muted-foreground">Nawigacja (jak w excelu):</div>
      ...
      {tab === "dane" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <Card title="Podstawowe dane">
            <KV k="Rodzaj" v={formatKind(s.kind)} />
            <KV k="Status" v={formatStatus(s.status)} />
            <KV k="E-mail" v={s.email} />
            <KV k="Telefon" v={s.phone} />
            <KV k="PESEL" v={s.pesel} />
            <KV k="NIP" v={s.nip} />
            <KV k="KRS" v={s.krs} />
            <KV k="REGON" v={s.regon} />
            <KV k="Utworzono" v={s.created_at} />
          </Card>

          <PlaceholderList
            title="KYC / dokumenty"
            hint="W excelu: skan dokumentu KYC + historia zmian. Tu placeholder UI." 
            items={[
              "Skan dokumentu KYC (upload) — placeholder",
              "Historia zmian danych abonenta (audit) — placeholder",
              "Weryfikacja tożsamości (status) — placeholder",
            ]}
          />
        </div>
      )}
      ...
      {tab === "uslugi" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          ...
          <PlaceholderList
            ...
            hint="To jest ten checkbox/wybór z UI usług: nie wymaga / NAT / public + ilość."
            items={[
      ...
      {tab === "zgody" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <Card title="Zgody (stan)" desc="RODO (wymagana), e‑faktury/panel, marketing + preferencje kanałów.">
            <KV k="RODO przetwarzanie" v={s.consents.rodo_processing ? "TAK" : "NIE"} />
            <KV k="E‑faktury / panel" v={s.consents.e_invoice ? "TAK" : "NIE"} />
            <KV k="Marketing" v={s.consents.marketing ? "TAK" : "NIE"} />
            <KV
              k="Kanały (operacyjne)"
              v={
                [
